...
| 2 | `pkg/auth/authorizer/abac/abac.go:180` | Verb-level matching in `verbMatches()` | TODO: `"match on verb"` — function currently allows all read-only requests without distinguishing specific verbs (GET, LIST, WATCH); all non-readonly requests are allowed if policy is not readonly | Verb-level granularity in ABAC policy enforcement is not operational, meaning fine-grained verb restrictions in ABAC policies are not enforced as expected | Moderate | 5.1 |
